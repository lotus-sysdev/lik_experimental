...
class MeasurementWidget(forms.MultiWidget):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        float_html = self.widgets[0].render(
            name + '_0', value[0], attrs=attrs, renderer=renderer
        )
        unit_html = self.widgets[1].render(
            name + '_1', value[1], attrs=attrs, renderer=renderer
        )

        # Add d-flex class wrapping input and select
        return mark_safe('<div class="d-flex" style="width:33%">{}</div>'.format(float_html + unit_html))
